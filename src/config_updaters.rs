...
fn register_config_updaters() -> JsonFileUpdater {
    ...
    json_config_updater.add_update_function("1.1.1", v1_1_3_add_immediate_script_keybind);
...
fn v1_0_2_add_file_associations(config_json: &mut JsonValue) {
    if let Some(rewritable_config) = config_json["rewritable"].as_object_mut() {
        rewritable_config.insert(
            "file_associations".to_string(),
            json!([{
                    "extension": "py",
                    "executor": ["python"],
                    "executor_arguments": [],
...
fn v1_1_0_rename_cursor_script_to_selected_scripts(config_json: &mut JsonValue) {
    if let Some(rewritable) = config_json["rewritable"].as_object_mut() {
        let Some(keybinds) = rewritable.get_mut("app_actions_keybinds") else {
            return;
        };

        let Some(keybinds) = keybinds.as_array_mut() else {
            return;
        };
